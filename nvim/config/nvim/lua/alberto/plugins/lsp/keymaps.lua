...
function M.get()
  local format = require("alberto.plugins.lsp.format").format
  if not M._keys then
  ---@class PluginLspKeys
    -- stylua: ignore
    M._keys =  {
      {"gf", "<cmd>Lspsaga lsp_finder<CR>", desc = "Search references"},
      ...
      { "gK", vim.lsp.buf.signature_help, desc = "Signature Help", has = "signatureHelp" },
	    ...
	    {"<leader>d", "<cmd>Lspsaga show_cursor_diagnostics<CR>", desc = "Show cursor diagnostics"},
      ...
      { "<F2>", M.diagnostic_goto(true, "ERROR"), desc = "Next Error" },
    ...
    if require("alberto.util").has("inc-rename.nvim") then
      M._keys[#M._keys + 1] = {
        "<leader>cr",
        function()
          require("inc_rename")
